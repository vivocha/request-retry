...
describe('Testing APIClient STATIC methods', function () {
  ...
  describe('APIClient.get()', function () {
    ...
    it('for a GET 302 redirection should not retry (1 call) and get a JSON from the Location: /api/get-test', async function () {
      const opts: APICallOptions = {
        json: true,
        retries: 3,
        retryAfter: 2000
      ...
      const spy = chai.spy.on(APIClient.prototype, 'call');
      const result = await APIClient.get('https://localhost:8443/api/redirect', opts);
      result.should.have.property('result');

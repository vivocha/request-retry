...
describe('Testing APIClient STATIC methods', function() {
  ...
  describe('APIClient.put()', function() {
    let env = process.env;
    ...
    it('for 200 should not retry (1 call)', async function() {
      ...
      const result = await APIClient.put('https://localhost:8443/api/things', body, {
        json: true,
        retries: 3,
        retryAfter: 2000
      });
    ...
    it('for not listed error (401) should retry 2 times (3 calls)', async function() {
      const body = { thingName: 'lamp', b: { b1: 'ok', b2: 'message' } };
      const spy = chai.spy.on(APIClient.prototype, 'call');
      await APIClient.put('https://localhost:8443/api/things-auth', body, {
        json: true,
        retries: 2,
        retryAfter: 1000
    ...
    it('with a put should use headers option preserving all headers, if set (1 call)', async function() {
      const body = { a: 'start', b: { b1: 'ok', b2: 'message' } };
      const hash = 'FFFFFFFF123456';
      const spy = chai.spy.on(APIClient.prototype, 'call');
      const result = await APIClient.put('https://localhost:8443/api/put-headers', body, {
        authOptions: {
    ...
    it('with a put passing a JSON string should use headers option preserving all headers, if set (1 call)', async function() {
      const body = { a: 'start', b: { b1: 'ok', b2: 'message' } };
      const sbody = JSON.stringify(body);
      const hash = 'FFFFFFFF123456';
      const spy = chai.spy.on(APIClient.prototype, 'call');
      const result = await APIClient.put('https://localhost:8443/api/put-string-headers', sbody, {
        authOptions: {
          authorizationType: 'Bearer',
          token: 'mytoken'
        },
    ...
    it('for a put request with Bearer token should return the correct header and body', async function() {
      const authorizationType = 'Bearer';
      const token = '123456';
      const body = { a: 1, b: 2 };
      const spy = chai.spy.on(APIClient.prototype, 'call');
      const result = await APIClient.put('https://localhost:8443/api/bearer-auth', body, {
    ...
    it('for a put request with bearer token (lowercase) should return the correct Bearer header and body', async function() {
      const authorizationType = 'bearer';
      const token = '123456';
      const body = { a: 1, b: 2 };
      const spy = chai.spy.on(APIClient.prototype, 'call');
    ...
    it('for a put request with Basic auth should return the correct header and body', async function() {
      const body = { a: 1, b: 2 };
      const spy = chai.spy.on(APIClient.prototype, 'call');
      const result = await APIClient.put('https://localhost:8443/api/put-basic', body, {
    ...
    it('for a put request with custom auth should use the correct header and body', async function() {
    ...
    it('for a put request with custom auth (lowercase) should use the correct preserved header and body', async function() {
      const authorizationType = 'app';
      const token = '123456';
      const body = { a: 1, b: 2 };
      const spy = chai.spy.on(APIClient.prototype, 'call');
    ...
    it('with a request timeout should be rejected', async function() {
      const body = { thingName: 'lamp', b: { b1: 'ok', b2: 'message' } };
      const spy = chai.spy.on(APIClient.prototype, 'call');
      return APIClient.put('https://localhost:8443/api/too-long-request', body, {
        json: true,
        retries: 5,
        retryAfter: 500,
        timeout: 2000
    ...
    it('with query params should use them properly (1 call)', async function() {
      ...
      const spy = chai.spy.on(APIClient.prototype, 'call');
      const result = await APIClient.put('https://localhost:8443/api/query-params?a=10&b=hello', {}, opts);
    ...
    it('with query params should use them properly (1 call)', async function() {
      ...
      const spy = chai.spy.on(APIClient.prototype, 'call');
      const result = await APIClient.put('https://localhost:8443/api/query-params', {}, opts);
    ...
    it('with no options should set 2 retries (3 calls) ', async function() {
      const spy = chai.spy.on(APIClient.prototype, 'call');
      await APIClient.put('https://localhost:8443/api/fourzerofour', '').should.eventually.be.rejected;
      return spy.should.have.been.called.exactly(3);
    });

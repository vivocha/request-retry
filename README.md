...
| `follow_max`              | (optional) number, a positive value enables redirects following; Default is `3` (enabled by default); setting it to `0`, disables following redirects. Inherited from Needle.                     |
| `follow_set_cookies`      | (optional) boolean, sends the cookies received in the `set-cookie` header as part of the following request, if hosts match. `false` by default. Inherited from Needle.                            |
| `follow_set_referer`      | (optional) boolean, sets the `Referer` header to the requested URI when following a redirect. `false` by default. Inherited from Needle.                                                          |
| `follow_keep_method`      | (optional) boolean, if enabled, resends the request using the original verb instead of being rewritten to get with no data. `true` by default. Inherited from Needle.                             |
| `follow_if_same_host`     | (optional) boolean, when `true`, will only follow redirects that point to the same host as the original request. `false` by default. Inherited from Needle.                                       |
| `follow_if_same_protocol` | (optional) boolean, when `true`,  will only follow redirects that point to the same protocol as the original request. `false` by default. Inherited from Needle.                                  |
| `follow_if_same_location` | (optional) boolean, unless `true`,  will not follow redirects that point to same location (as set in the response header) as the original request URL. `false` by default. Inherited from Needle. |
